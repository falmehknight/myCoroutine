...
#undef XX
    ...
    // socket⽤于创建套接字，需要在拿到fd后将其添加到FdManager中
    ...
    int connect_with_timeout(int fd, const struct sockaddr* addr, socklen_t addrlen, uint64_t timeout_ms) {
        ...
        // 不是socket以及用户设置了非阻塞的情况，都可以直接返回原系统调用
        if(!ctx->isSocket()) {
            return connect_f(fd, addr, addrlen);
        }
        ...
        // 调⽤系统的connect函数，由于套接字是⾮阻塞的，这⾥会直接返回n = -1 且errno为EINPROGRESS错误(非阻塞操作正在进行中)
        ...
        std::weak_ptr<timer_info> winfo(tinfo);
        // 2. 超时设置
        /// 等待超时或者套接字可写，如果先超时，则winfo还有效，通过winfo来设置超时标志并触发WRITE事件，协程从yield点返回，
        /// 返回后通过超时标志设置errno并返回-1；如果在未超时之前套接字就可写了，那么直接取消定时器并返回成功
        ...
        if(rt == 0) {
            ...
            if(timer) {
                ...
                /// 由于定时器回调被执⾏时connect_with_timeout协程已经执⾏完了，所以理所当然地条件变量也被释放了，所以实际上定时器回调函数什么也没做。
                timer->cancel();
            }
            // 先超时了，则返回超时。
            if(tinfo->cancelled) {
